...
from models.resnets import resnet50gntimm
import torch
import torchvision
...
if __name__ == "__main__":
    ...
    model_dir = os.path.join(model_dir, f'resnet50gntimm', 'pairsplits')
    print(model_dir)
    ...
    for _ in range(split_runs):
        ...
        for j in range(models_per_run):
            for i in range(len(splits)):
                model = resnet50gntimm(num_classes=out_dim).cuda().train()
                ...
                save_path = os.path.join(
                    save_dir,
                    f'resnet50gntimm_v{len(os.listdir(save_dir))}.pth.tar')
